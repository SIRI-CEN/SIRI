...
declare function sd:castToComparable($s as xs:string?)
    ...
    (: Workaround for BaseX <12.1 which cannot compare xs:duration values.
       Use ordered subtypes instead. See https://github.com/SIRI-CEN/SIRI/issues/218 :)
    else if ($s castable as xs:dayTimeDuration) then xs:dayTimeDuration($s)
    else if ($s castable as xs:yearMonthDuration) then xs:yearMonthDuration($s)
